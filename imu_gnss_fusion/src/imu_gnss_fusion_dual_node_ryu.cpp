...
class FusionNode {
   public:
    FusionNode(ros::NodeHandle &nh) {
        ...
        double gps1_x_imuFrame, gps1_y_imuFrame, gps1_z_imuFrame;
        double gps2_x_imuFrame, gps2_y_imuFrame, gps2_z_imuFrame;
        ...
        nh.param("gps1_x_imuFrame", gps1_x_imuFrame, 0.);
        nh.param("gps1_y_imuFrame", gps1_y_imuFrame, 0.);
        nh.param("gps1_z_imuFrame", gps1_z_imuFrame, 0.);
        nh.param("gps2_x_imuFrame", gps2_x_imuFrame, 0.);
        nh.param("gps2_y_imuFrame", gps2_y_imuFrame, 0.);
        nh.param("gps2_z_imuFrame", gps2_z_imuFrame, 0.);
        ...
        std::string topic_imu, topic_gps1, topic_gps2;
        nh.param<std::string>("topic_imu", topic_imu, "/imu/data");
        nh.param<std::string>("topic_gps1", topic_gps1, "/gnss_1/llh_position");
        nh.param<std::string>("topic_gps2", topic_gps2, "/gnss_2/llh_position");
        ...
        I_p_Gps1_ = Eigen::Vector3d(gps1_x_imuFrame, gps1_y_imuFrame, gps1_z_imuFrame);
        I_p_Gps2_ = Eigen::Vector3d(gps2_x_imuFrame, gps2_y_imuFrame, gps2_z_imuFrame);

        // ROS sub & pub
        imu_sub_ = nh.subscribe(topic_imu, 10, &FusionNode::imu_callback, this);
        gps1_sub_ = nh.subscribe(topic_gps1, 10, &FusionNode::gps1_callback, this);
        gps2_sub_ = nh.subscribe(topic_gps2, 10, &FusionNode::gps2_callback, this);
    ...
    void gps1_callback(const sensor_msgs::NavSatFixConstPtr &gps_msg);
    void gps2_callback(const sensor_msgs::NavSatFixConstPtr &gps_msg);
    void gps_process(const sensor_msgs::NavSatFixConstPtr &gps_msg, const Eigen::Vector3d& I_p_Gps_);
   ...
   private:
    ...
    ros::Subscriber gps1_sub_;
    ros::Subscriber gps2_sub_;
    ...
    Eigen::Vector3d I_p_Gps1_;
    Eigen::Vector3d I_p_Gps2_;
...
void FusionNode::gps1_callback(const sensor_msgs::NavSatFixConstPtr &gps_msg) {
    gps_process(gps_msg, I_p_Gps1_);
}

void FusionNode::gps2_callback(const sensor_msgs::NavSatFixConstPtr &gps_msg) {
    gps_process(gps_msg, I_p_Gps2_);
}

void FusionNode::gps_process(const sensor_msgs::NavSatFixConstPtr &gps_msg, const Eigen::Vector3d& I_p_Gps_) {
