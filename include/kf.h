...
class KF {
   public:
    ...
    void predict(ImuDataConstPtr last_imu, ImuDataConstPtr curr_imu) {
        ...
        Fx.block<3, 3>(3, 6) = -state_ptr_->r_GI * ryu::skew_matrix(acc_unbias) * dt;
    ...
    void update_6DOF(const Eigen::Vector3d& measured_position, const Eigen::Matrix3d& measured_orientation, const Eigen::Matrix3d& residual_orientation,\
        ...
        // H2.block<3, 3>(0, 6) = -R_identity * T_predict.linear() * ryu::skew_matrix(T_gnss_imuFrame.inverse().translation());
        // H2.block<3, 3>(3, 6) = (ryu::quat_left_matrix((q_identity * q_predict).normalized()) * ryu::quat_right_matrix(q_gnss_imu_frame.conjugate())).block<3, 3>(1, 1);
        // H.block<3, 3>(0, 0) = R_identity;
        // H.block<3, 3>(0, 6) = -R_identity * T_predict.linear() * ryu::skew_matrix(T_gnss_imuFrame.inverse().translation());
        // H.block<3, 3>(3, 6) = (ryu::quat_left_matrix((q_identity * q_predict).normalized()) * ryu::quat_right_matrix(q_gnss_imu_frame.conjugate())).block<3, 3>(1, 1);
        ...
        H.block<3, 3>(0, 0) = -R_identity;
        H.block<3, 3>(0, 6) = R_identity * T_predict.linear() * ryu::skew_matrix(T_gnss_imuFrame.inverse().translation());
        m4 = ryu::quat_left_matrix((measured_q.conjugate() * q_identity * q_predict).normalized()) * ryu::quat_right_matrix(q_gnss_imu_frame.conjugate());
    ...
    void update_3DOF(const Eigen::Vector3d& measured_position, const Eigen::Matrix3d& cov, const Eigen::Vector3d& gnss_position_imuFrame) {
        ...
        H.block<3, 3>(0, 6) = -state_ptr_->r_GI * ryu::skew_matrix(gnss_position_imuFrame);
