...
template <class H_type, class Res_type, class R_type>
void update_delay_measurement(
    StatePtr state_ptr, const Eigen::MatrixBase<H_type> &H, const Eigen::MatrixBase<R_type> &V, const Eigen::MatrixBase<Res_type> &r) {
    EIGEN_STATIC_ASSERT_FIXED_SIZE(H_type);
    EIGEN_STATIC_ASSERT_FIXED_SIZE(R_type);

    // compute K
    const MatrixSD &P = state_ptr->cov;
    const R_type S = H * P * H.transpose() + V;
    const Eigen::Matrix<double, kStateDim, R_type::RowsAtCompileTime> K = P * H.transpose() * S.inverse();
    ...
    // update: nominal-state + observation error
    state_ptr->p_GI += delta_x.block<3, 1>(0, 0);
    state_ptr->v_GI += delta_x.block<3, 1>(3, 0);
    const Eigen::Vector3d &dR = delta_x.block<3, 1>(6, 0);
    if (dR.norm() > DBL_EPSILON) {
        state_ptr->r_GI *= Eigen::AngleAxisd(dR.norm(), dR.normalized()).toRotationMatrix();
    }
    state_ptr->acc_bias += delta_x.block<3, 1>(9, 0);
    state_ptr->gyr_bias += delta_x.block<3, 1>(12, 0);

    // update: error-state covariance
    const MatrixSD I_KH = MatrixSD::Identity() - K * H;
    state_ptr->cov = I_KH * P * I_KH.transpose() + K * V * K.transpose();
}
